...
final String[][] surveyData = new String[1][];
PrintWriter languageInput;
PrintWriter aiGuess;
...
int[] countedLanguages = {3,4,5,6,8};
boolean lastOneWasCorrect = false;
String[] languages = {"Random","Key Mash","Engelska","Spanska","Franska","Tyska","Japanska",
...
void setup(){
  ...
  languageInput = createWriter("Wordlist.txt");
  aiGuess = createWriter("AI Guess.txt");
  
  surveyData[0] = loadStrings("SurveyWordlist.txt");
...
void draw(){
  ...
  if(keyPressed){
    ...
    if(c == 49 && lastPressedKey != 49){
      ...
      if(brain.alpha>=0.01) brain.alpha += 0.005;
      else if(brain.alpha<0.01) brain.alpha += 0.0005;
    }
  ...
  text("7 to run through wordlist.",ex,700);
  if(smooth == 1){
...
void AIGuess(){
  word = "";
  aiGuess.println(iteration);
  int wordIndex = 0;
  while(wordIndex < 105){
    word = surveyData[0][wordIndex];
    getBrainErrorFromLine(word,0,false);
    aiGuess.println(word + "," + languages[brain.topOutput] + "," + brain.confidence);
    wordIndex++;
    delay(20);
  }
  aiGuess.flush();
...
void train(){
  ...
  languageInput.println(word + "," + languages[lang]);
